---
name: writing-skills
description: ALWAYS invoke when the user asks to "write a skill", "author a new skill", "create a SKILL.md", or "add a skill to the plugin". Enforces Rules 16-21 of SKILL_GUIDELINES.md via a TDD-style authoring workflow.
...
1. **Classify skill type** per SKILL_GUIDELINES.md Rule 16: discipline (pushes back against shortcuts), collaboration (shared output with the user), or reference (neutral knowledge). The classification determines the persuasion register the skill must use and whether Rule 17 requires an anti-rationalization table.
...
N/A — collaboration skill per Rule 17, not discipline.
...
- The description reads like a workflow summary ("Orchestrates X, then Y, then Z") instead of a CSO trigger list. Rule 18 bans this — descriptions are the skill's auction bid for invocation, not a table of contents.
- You skipped the persuasion-register check from Rule 20. Discipline skills must use Authority/Commitment/Social-Proof framing; Liking language is banned for them because it drops compliance from 72% to 33% per Meincke et al. (2025).

## Verification

Run the Rule 21 pre-publish checklist at the end of Phase 1. Every bullet must be green before the skill is considered done:

- Description is CSO-compliant (Rule 18), ≤250 chars, triggers only, no workflow tail.
- SKILL.md ≤150 lines; phase files ≤400 lines.
- All 7 canonical sections present (Rule 16), in order.
- Anti-rationalization table present with ≥3 rows if discipline skill (Rule 17).
- Persuasion register matches skill type (Rule 20); no Liking language in discipline skills.
