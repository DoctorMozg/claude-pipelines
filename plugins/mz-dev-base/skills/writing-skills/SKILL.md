---
name: writing-skills
description: ALWAYS invoke when the user asks to "write a skill", "author a new skill", "create a SKILL.md", or "add a skill to the plugin". Enforces Rules 16-24 of SKILL_GUIDELINES.md via a TDD-style authoring workflow.
...
1. **Classify skill type** per SKILL_GUIDELINES.md Rule 23: discipline (pushes back against shortcuts), collaboration (shared output with the user), or reference (neutral knowledge). The classification determines the persuasion register the skill must use and whether Rule 17 requires an anti-rationalization table.
...
- You skipped the persuasion-register check from Rule 23. Discipline skills must use Authority/Commitment/Social-Proof framing; Liking language is banned for them because it drops compliance from 72% to 33% per Meincke et al. (2025).

## Verification

Run the Rule 24 pre-publish checklist at the end of Phase 1. Every bullet must be green before the skill is considered done:
...
- Persuasion register matches skill type (Rule 23); no Liking language in discipline skills.
