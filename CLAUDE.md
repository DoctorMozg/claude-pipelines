...
- **State lives at `.mz/task/<task_name>/state.md`.** The task-name pattern is `<YYYY_MM_DD>_<skill>_<slug>` (e.g. `2026_04_20_build_oauth_flow`). On same-day collision append `_v2`, `_v3`. All generated reports under `.mz/reports/`, `.mz/reviews/`, `.mz/research/`, `.mz/design/`, and `.mz/outreach/` follow the same `<YYYY_MM_DD>_<skill>_<slug>` leading-date convention. Never rely on conversation memory for cross-phase state — context compaction destroys it.
- **Interaction journal at `.mz/journal.md`.** One global, append-only, gitignored record of every approval gate, input prompt, and recorded decision across all skills. A `mz-memory` `PostToolUse` hook on `AskUserQuestion` captures every gate verbatim automatically (the audit guarantee); skills SHOULD add semantic enrichment (skill, phase, artifact) and promote substantial decisions into recall memory by writing a `## Decisions` section in `state.md` (harvested into `.mz/memory/MEMORY.md` at SessionEnd). See `guidelines/JOURNAL_GUIDELINES.md` for the format, the two-layer capture model, and the `<private>` redaction convention.
...
- **Approval gates use the two-surface plan pattern.** Modeled on Claude Code's plan mode: every user approval gate emits the artifact's full verbatim contents as a normal rendered markdown chat message (Surface 1 — the way a plan appears in plan mode), then calls AskUserQuestion as a short selector (Surface 2) with exactly two named options — **Approve** and **Reject**. The verbatim artifact never goes inside the AskUserQuestion body. Feedback rides AskUserQuestion's always-present free-text reply field; there is no separate "Feedback" option, and the old lowercase `Reply 'approve'…` form is banned. On feedback the orchestrator overwrites the artifact and re-emits the entire updated plan message from scratch. Variant gates (menus with extra named actions) present each action as `**<Name>** — <one-sentence summary>`. See `guidelines/SKILL_GUIDELINES.md` §1 and the reference implementation at `plugins/mz-design/skills/design-document/phases/finalization.md` Step 4.1.
